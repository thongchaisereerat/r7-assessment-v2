...
const API='https://script.google.com/macros/s/AKfycbzOfoslb872k5viPjuVwaQJE21jc9sV_38S9Y3uq0LS1jTrIPQb9XXRjMZCtBUTOZ3V1g/exec';
  ...
  try{
    ...
    if(s.success&&s.scores&&s.scores.length>0){
      ...
      AS=s.scores.map(sc=>{
        ...
        }else if(sc.composite!==undefined){
          // Use server's weighted score directly
          sc.wpct=Number(sc.composite);
          sc.grade=sc.grade||gradeFromPct(sc.wpct);
        }else{
          // Last fallback: estimate from raw score
...
async function saveAssess(){
  ...
  gAPI('getAllScores').then(sr=>{if(sr.success&&sr.scores&&sr.scores.length>0){
    sr.scores.forEach(sc=>{if(sc.wpct===undefined&&sc.composite!==undefined)sc.wpct=Number(sc.composite);if(sc.wpct===undefined)sc.wpct=0;if(!sc.grade)sc.grade=gradeFromPct(sc.wpct);});
    const hist=processHistorical();AS=sr.scores.concat(hist);
  }}).catch(()=>{});
